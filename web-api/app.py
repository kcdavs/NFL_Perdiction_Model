...
def extract_metadata(year, week):
    ...
    for row in soup.find_all("tr", class_="participantRow--z17q"):
        ...
        # --- begin fix: only remap Oakland before 2020 ---
        team_tag = row.find("div", class_="participantName-3CqB8")
        raw_team = team_tag.get_text(strip=True) if team_tag else ""
        if year < 2020 and "Oakland" in raw_team:
            team = "Las Vegas"
        else:
            team = raw_team
        # --- end fix ---
...
def load_and_pivot_acl(filepath, label):
    ...
    suffixes = sorted(set(re.sub(r"^\D+_", "", col) for col in adj_cols + ap_cols),
